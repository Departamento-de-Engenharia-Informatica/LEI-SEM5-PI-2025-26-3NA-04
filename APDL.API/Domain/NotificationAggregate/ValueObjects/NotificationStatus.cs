     ...
     public class NotificationStatus : IValueObject
    {
        ...
        public NotificationStatus(string value, bool validate = true)
        {
            ...
            if (string.IsNullOrWhiteSpace(normalizedValue))
                throw new BusinessRuleValidationException(nameof(NotificationStatus), "Status cannot be empty.");

            if (!normalizedValue.Equals("InProgress", StringComparison.OrdinalIgnoreCase) &&
            ...
            {
                var s when s.Equals("InProgress", StringComparison.OrdinalIgnoreCase) => "InProgress",
